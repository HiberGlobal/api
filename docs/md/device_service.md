...
|  **optional** transmission_interval | [optional hiber.Duration](#hiberduration) | Sets the interval this devices is transmitting on. For devices that are capable of remote configuration, this will also (try to) update the configuration on the device. If the device (or Mission Control) does not support remote configuration, this field is considered documentation. |
